...
import text_sed.diffusion as diffusion
import text_sed.layers as layers
import text_sed.utils as utils
import text_sed.slurm as slurm


logger = logging.getLogger(__name__)


def train(config: oc.DictConfig):
    ...
    logger.info(
        f"Parameter count: ~{format(utils.param_count(model), ',')}")
    ...
    for step in tqdm(
        range(step_state, config.train.max_steps),
        ...
        if step % config.train.sample_every == 0 and step != 0:
            ...
            for sample in samples:
                if sample is None:
                    sample_log += "None\n"
                    continue
                sample_log += f"➜ {tokenizer.decode(sample, skip_special_tokens=True)}\n"
